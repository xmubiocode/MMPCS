...
for i in tqdm(range(epoch)):
    ...
    for smiles, graph, label in train_bar:
        ...
        try:
            smiles_rep, graph_rep, recon_loss, output = model(smiles, graph)
        except:
            print("smiles", smiles)
            print("graph", graph)
            print("label", label)
        ...
        loss_info_nce = 0
        print(f" loss_cov_smiles:{loss_cov_smiles.item():.4f}, loss_cov_graph:{loss_cov_graph.item():.4f}, recon_loss:{recon_loss.item():.4f}, loss_regression:{loss_regression.item():.4f}")
        
        loss = loss_info_nce + 1 * (loss_cov_smiles + loss_cov_graph) + 1 * (recon_loss) + 10 * (loss_regression)
        loss.backward()
        optimizer.step()
        # print(loss.item())
    ...
    with torch.no_grad():
        ...
        for smiles, graph, labels in tqdm(test_dataloader):
            graph = graph.to(device)
            # label = label.to(device)
            _, _, _, output = model(smiles, graph)
