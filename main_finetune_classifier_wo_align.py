...
from finetune_model import FineTuneModelClassifierWoAe
...
# 创建模型
model = FineTuneModelClassifierWoAe(datasetloadhelper=datasetloadhelper, device=device, is_multi_gpu=is_multi_gpu)
model_dir = "model_wo_align_2024-02-26-11-05-54"
...
for i in range(epoch):
    ...
    for smiles, graph, labels in train_bar:
        ...
        smiles_rep, graph_rep, outputs = model(smiles, graph)
        # print(labels)
        # print(outputs)
        # pooling 
        ...
        loss_info_nce = 0
        print(f" loss_cov_smiles:{loss_cov_smiles.item():.4f}, loss_cov_graph:{loss_cov_graph.item():.4f}, loss_classification:{loss_classification_sum.item():.4f}")
        
        loss = loss_info_nce + 1 * (loss_cov_smiles + loss_cov_graph) + 10*(loss_classification_sum)
        loss.backward()
        optimizer.step()
        # print(loss.item())
    ...
    with torch.no_grad():
        for smiles, graph, labels in tqdm(test_dataloader):
            graph = graph.to(device)
            # label = label.to(device)
            _, _, outputs = model(smiles, graph)
...
if (is_multi_gpu == True and dist.get_rank() == 1) or is_multi_gpu == False:
    ...
    #                 df_labels_list = [[] for i in range(len(labels))]
    #             for index, label in enumerate(labels):
    #                 df_labels_list[index].extend(label.tolist())
    ...
    # df_lable = pd.DataFrame(df_labels_list)
    # # print("df_embedding", df_embedding)
    # print("df_lable", df_lable)
    # df_embedding.to_csv(os.path.join(dir_name, f"df_embedding_{dataset_name}.csv"), index=False)
    # df_lable.to_csv(os.path.join(dir_name, f"df_label_{dataset_name}.csv"), index=False)
