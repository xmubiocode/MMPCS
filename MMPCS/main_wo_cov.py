...
import os
# 创建文件夹
dir_name = f"model_wo_cov_{time.strftime('%Y-%m-%d-%H-%M-%S',time.localtime(time.time()))}"
os.mkdir(dir_name)
...
for i in range(epoch):
    ...
    for smiles, graph in train_bar:
        ...
        loss_info_nce = 0
        print(f"recon_loss_smiles_to_graph:{recon_loss_smiles_to_graph.item():.4f}, recon_loss_graph_to_smiles:{recon_loss_graph_to_smiles.item():.4f}")
        print(f" loss_fu_smiles:{loss_fu_smiles.item():.4f}, loss_fu_graph:{loss_fu_graph.item():.4f}")
        loss = loss_info_nce + 1*(recon_loss_smiles_to_graph + recon_loss_graph_to_smiles)+loss_fu_smiles+loss_fu_graph
        loss.backward()
        optimizer.step()
        # print(loss.item())
