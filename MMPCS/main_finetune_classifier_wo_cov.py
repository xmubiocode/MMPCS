...
model_dir = "model_wo_cov_2024-02-26-09-59-21"
...
for i in range(epoch):
    ...
    for smiles, graph, labels in train_bar:
        ...
        loss_info_nce = 0
        print(f"recon_loss:{recon_loss.item():.4f}, loss_classification:{loss_classification_sum.item():.4f}")
        
        loss = loss_info_nce + 1 * (recon_loss) + 10*(loss_classification_sum)
        loss.backward()
        optimizer.step()
        # print(loss.item())
