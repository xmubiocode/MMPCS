...
import os
# 创建文件夹
dir_name = os.path.join(model_dir, f"finetune_epoch_{model_index}_gnn")
...
for i in range(epoch):
    ...
    for smiles, graph, labels in train_bar:
        ...
        outputs = model.forward_gnn(smiles, graph)
        # print(labels)
        # print(outputs)
        # pooling 
        ...
        loss_info_nce = 0
        print(f" loss_classification:{loss_classification_sum.item():.4f}")
        
        loss = loss_classification_sum
        loss.backward()
        optimizer.step()
        # print(loss.item())
