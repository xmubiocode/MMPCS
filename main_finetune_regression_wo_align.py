...
# 自己写的
from finetune_model import FineTuneModelRegressionWoAe
...
# 创建模型
model = FineTuneModelRegressionWoAe(device=device)
...
for i in tqdm(range(epoch)):
    ...
    for smiles, graph, label in train_bar:
        ...
        try:
            smiles_rep, graph_rep, output = model(smiles, graph)
        except:
            print("smiles", smiles)
            print("graph", graph)
            print("label", label)
        ...
        loss_info_nce = 0
        print(f" loss_cov_smiles:{loss_cov_smiles.item():.4f}, loss_cov_graph:{loss_cov_graph.item():.4f}, loss_regression:{loss_regression.item():.4f}")
        
        loss = loss_info_nce + 1 * (loss_cov_smiles + loss_cov_graph) + 10 * (loss_regression)
        loss.backward()
        optimizer.step()
        # print(loss.item())
